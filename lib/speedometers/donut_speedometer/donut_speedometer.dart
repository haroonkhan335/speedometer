...
import 'package:speedometer/speedometers/donut_speedometer/paint.dart';

final SpeedometerConfig defaultDonutSpeedometerConfig = DonutSpeedometerConfig(
  colorType: SpeedometerColorType.solid,
);

class DonutSpeedometer extends Speedometer {
  final double maxSpeed;
  final double initialValue;
  final SpeedometerConfig? config;

  const DonutSpeedometer({
    super.key,
  ...
  _DonutSpeedometerState createState() => _DonutSpeedometerState();
}

class _DonutSpeedometerState extends State<DonutSpeedometer> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) => CustomPaint(
        isComplex: true,
        willChange: true,
        size: widget.config?.size ?? defaultDonutSpeedometerConfig.size,
        painter: DonutSpeedometerPaint(
          animation: _animation,
          maxSpeed: widget.maxSpeed,
          initialValue: widget.initialValue,
          config: widget.config as DonutSpeedometerConfig? ??
              defaultDonutSpeedometerConfig as DonutSpeedometerConfig,
        ),
      ),
    );
  }
